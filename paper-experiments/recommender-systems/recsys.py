...
def get_model(model_name, training_method, features_dimension, dropout, device):
    ...
    if model_name == 'fm':
        ...
        if training_method_signature == 'ALT':
            model = DFADeepFactorizationMachineModel(features_dimension, embed_dim=10, mlp_dims=(400, 400, 400),
                                                     dropout=dropout, training_method=training_method)
        else:
            model = DeepFactorizationMachineModel(features_dimension, embed_dim=10, mlp_dims=(400, 400, 400),
                                                  dropout=dropout)
        ...
        if training_method_signature == 'ALT':
            ...
            model = DeepCrossNetworkModel(features_dimension, embed_dim=16, num_layers=6, mlp_dims=(1024, 1024),
                                          dropout=dropout)
        ...
        if training_method_signature == 'ALT':
            ...
            model = AutomaticFeatureInteractionModel(features_dimension, embed_dim=16, atten_embed_dim=32, num_heads=2,
                                                     num_layers=3, mlp_dims=(400, 400, 400),
                                                     dropouts=(dropout, dropout, dropout))
    elif model_name == 'afn':
        if training_method_signature == 'ALT':
            ...
            model = AdaptiveFactorizationNetwork(features_dimension, embed_dim=10, LNN_dim=1500,
                                                 mlp_dims=(400, 400, 400), dropouts=(dropout, dropout, dropout))

    if isinstance(model, DeepCrossNetworkModel) or isinstance(model, DFADeepCrossNetworkModel):
        print("Using a DeepCross model, will use gradient clip norm=100. during training.")

    return model.to(device)


def train(model, optimizer, data_loader, criterion, device, verbose, print_every=1000):
    model.train()
...
def main(model_name, training_method, dataset_name, dataset_dir, epoch, learning_rate, batch_size, dropout,
         weight_decay, patience, gpu_id, save_dir, verbose):

    device = get_device(gpu_id)
    features_dimension, (train_data_loader, validation_data_loader, test_data_loader) = get_loaders(dataset_name,
                                                                                                    dataset_dir,
                                                                                                    batch_size)
    model = get_model(model_name, training_method, features_dimension, dropout, device)

    criterion = torch.nn.BCELoss()
    optimizer = torch.optim.Adam(params=model.parameters(), lr=learning_rate, weight_decay=weight_decay)

    timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d_%H%M%S')
    signature = f'{timestamp}__{model_name}_{training_method}'
    ...
    for e in range(epoch):
        train(model, optimizer, train_data_loader, criterion, device, verbose)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, choices=['fm', 'deepfm', 'deepcross', 'autoint', 'afn'], default='deepfm',
                        help='model to use, choose from Factorization Machine (fm), Deep Factorization Machine (dfm), '
    ...
    parser.add_argument('--dataset-name', type=str, choices=['criteo', 'movielens1m'], default='criteo',
                        help='dataset on which to train, choose between the Kaggle Criteo dataset (criteo)'
                             'and the MovieLens-1M dataset (movielens-1M) (default: movielens-1M)')
    ...
    np.random.seed(42)
    main(args.model, args.training_method, args.dataset_name, args.dataset_dir, args.epoch, args.learning_rate,
